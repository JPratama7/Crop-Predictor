...
def main():
    ...
    df['Luas Panen'] = df['Luas Panen'].interpolate(method='polynomial', order=2)
    df['Hasil Panen'] = df['Hasil Panen'].interpolate(method='polynomial', order=2)
    df['Produktivitas'] = df['Produktivitas'].interpolate(method='polynomial', order=2)
    df['Humidity'] = df['Humidity'].interpolate(method='polynomial', order=2)
